...
def _soft_four_vector_features(soft_four_vectors: Any, estimated_pt_fraction: Any, eps: float) -> Any:
    ...
    safe_phi_px = torch.where(pt > float(eps), px, px.new_full(px.shape, float(eps)))
    safe_phi_py = torch.where(pt > float(eps), py, torch.zeros_like(py))
    phi = torch.atan2(safe_phi_py, safe_phi_px)

"""Training loop for the teacher-logit P-CNN-style reconstructor."""
...
EXPERIMENT_STEP = "teacher_logit_reco_step5_particle_cnn_train"
RECONSTRUCTOR_ARCHITECTURE = "particle_cnn"


@dataclass
class TeacherLogitParticleCnnTrainConfig:
    """Configuration for Step 5 P-CNN-style reconstructor training."""
    ...
    def model_config(self) -> ParticleCnnReconstructorConfig:
        return ParticleCnnReconstructorConfig(
...
def teacher_logit_particle_cnn_checkpoint_payload(
    model,
    optimizer,
    *,
    epoch: int,
    config: TeacherLogitParticleCnnTrainConfig,
    model_config: ParticleCnnReconstructorConfig,
...
def train_teacher_logit_particle_cnn_reco(
    config: TeacherLogitParticleCnnTrainConfig,
    *,
    model=None,
    teacher=None,
    ...
    """Train the Step 5 teacher-logit P-CNN-style reconstructor."""
    ...
    reconstructor = model or ParticleCnnReconstructor(model_config)
    ...
    run_metadata = {
        ...
        "leakage_rule": (
            "Step 5 trains the P-CNN-style reconstructor only on model_train and selects only on model_val. "
    ...
    if best_epoch < 0 or not (output_dir / "best_model_val.pt").exists():
        raise FloatingPointError(
            "Teacher-logit P-CNN reconstructor did not produce a finite model_val total_loss, "

...
HLT_MV_TRIVIEW_DEFAULT_STRENGTHS = (HLT_MV_TRIVIEW_S0P35_STRENGTH, HLT_MV_TRIVIEW_S1P00_STRENGTH)
_HLT_MV_TRIVIEW_RE = re.compile(r"^tri_hlt_hlt2_(?P<first>s\d+p\d+)_(?P<second>s\d+p\d+)$")
...
HLT_MV_TRIVIEW_DEFAULT_HEAD_WARMUP_LR = 3.0e-4
...
def hlt_mv_triview_strengths_from_name(
    model_name: str = HLT_MV_TRIVIEW_MODEL_NAME,
    *,
    config: HLTMVExperimentConfig | None = None,
) -> tuple[float, float]:
    """Return the two HLT2 strengths encoded in a tri-view model name."""

    name = normalize_hlt_mv_source_name(model_name)
    match = _HLT_MV_TRIVIEW_RE.match(name)
    if not match:
        raise ValueError(
            "HLT-MV tri-view model must look like "
            f"'tri_hlt_hlt2_sXpYY_sXpYY', got {model_name!r}."
        )
    first = normalize_hlt_sdv_strength(match.group("first"))
    second = normalize_hlt_sdv_strength(match.group("second"))
    if first == second:
        raise ValueError(f"HLT-MV tri-view strengths must be distinct, got {model_name!r}.")
    cfg = _config_for(config)
    available = set(cfg.strengths)
    if first not in available or second not in available:
        raise ValueError(
            f"HLT-MV tri-view strengths {(first, second)!r} are not both in configured grid {cfg.strengths!r}."
        )
    return first, second


def normalize_hlt_mv_triview_name(
    model_name: str = HLT_MV_TRIVIEW_MODEL_NAME,
    *,
    config: HLTMVExperimentConfig | None = None,
) -> str:
    """Validate an HLT-MV tri-view model name."""

    name = normalize_hlt_mv_source_name(model_name)
    hlt_mv_triview_strengths_from_name(name, config=config)
    return name
...
def hlt_mv_triview_source_names(
    *,
    ...
    """Return the canonical HLT and two matching HLT2 source names."""

    cfg = _config_for(config)
    first, second = hlt_mv_triview_strengths_from_name(model_name, config=cfg)
    return (
        cfg.canonical_hlt_source_name,
        hlt_mv_hlt2_source_name(first, seeds_by_strength=cfg.hlt2_source_seeds),
        hlt_mv_hlt2_source_name(second, seeds_by_strength=cfg.hlt2_source_seeds),
    )
...
def hlt_mv_triview_checkpoint_paths(
    ...
    *,
    ...
    hlt_source, hlt2_s0p35_source, hlt2_s1p00_source = hlt_mv_triview_source_names(
...
def hlt_mv_triview_output_dir(
    ...
    *,
    ...
    return layout.triview_model_dir(normalize_hlt_mv_triview_name(model_name, config=config))
...
def hlt_mv_triview_prediction_paths(
    ...
    *,
    ...
    name = normalize_hlt_mv_triview_name(model_name, config=config)
...
def build_hlt_mv_triview_config(
    *,
    ...
    name = normalize_hlt_mv_triview_name(model_name, config=cfg)
    lay = _layout_for(
        layout=layout,
        output_root=output_root,
    ...
    first_strength, second_strength = hlt_mv_triview_strengths_from_name(name, config=cfg)
    ...
    return HLTTriViewTrainConfig(
        output_dir=str(Path(output_dir) if output_dir is not None else hlt_mv_triview_output_dir(lay, name, config=cfg)),
...
def train_hlt_mv_triview(config: HLTTriViewTrainConfig) -> dict[str, Any]:
    ...
    first_strength, second_strength = hlt_mv_triview_strengths_from_name(config.model_name)
    ...
    hlt_mv_report = {
        **report,
        ...
        "hlt_mv_triview_strengths": [first_strength, second_strength],
        "hlt_mv_triview_strength_tags": [hlt_sdv_strength_tag(first_strength), hlt_sdv_strength_tag(second_strength)],
        "hlt_mv_branch_initializers": {
            "hlt": str(config.hlt_source_checkpoint),
            f"hlt2_{hlt_sdv_strength_tag(first_strength)}": str(config.hlt2_s0p35_source_checkpoint),
            f"hlt2_{hlt_sdv_strength_tag(second_strength)}": str(config.hlt2_s1p00_source_checkpoint),
        },
...
__all__ = [
    ...
    "HLT_MV_TRIVIEW_DEFAULT_STRENGTHS",
    ...
    "hlt_mv_triview_strengths_from_name",

"""Step 14 final reports for local-compression feature-adapter ParT runs."""
...
from .train import LOCAL_COMPRESSION_LOWER_IS_BETTER_SELECTION_METRICS, LOCAL_COMPRESSION_SELECTION_METRICS


LOCAL_COMPRESSION_REPORT_STEP = "local_compression_part_step14_reports"
LOCAL_COMPRESSION_REPORT_CONTRACT = f"{LOCAL_COMPRESSION_PART_CONTRACT}_report_v1"
LOCAL_COMPRESSION_REPORT_SPLITS = ("model_val", "stack_val", "final_test")
LOCAL_COMPRESSION_REPORT_BINARY_METRICS = (
    "auc",
...
def _variant_from_report(report_path: Path, report: Mapping[str, Any] | None) -> str:
    if isinstance(report, Mapping):
        for key in ("variant", "name"):
            ...
            if value:
                return normalize_local_compression_variant(str(value))
        config = report.get("config")
        if isinstance(config, Mapping) and config.get("variant"):
            return normalize_local_compression_variant(str(config["variant"]))
    return normalize_local_compression_variant(report_path.parent.name)
    ...
    *,
    ...
    if isinstance(child_reports, Mapping):
        for variant, payload in child_reports.items():
            ...
            if path is not None:
                paths[normalize_local_compression_variant(str(variant))] = path
    ...
    requested = [normalize_local_compression_variant(variant) for variant in variants if str(variant)]
    if requested:
        for variant in requested:
            paths.setdefault(variant, experiment_dir / variant / "run_report.json")
    elif not paths:
        for variant in LOCAL_COMPRESSION_DEFAULT_PILOT_VARIANTS:
            paths.setdefault(variant, experiment_dir / variant / "run_report.json")
...
def _metric_direction(metric_name: str) -> str:
    return "minimize" if metric_name in LOCAL_COMPRESSION_LOWER_IS_BETTER_SELECTION_METRICS else "maximize"
...
def _prediction_arrays(metrics: Mapping[str, Any] | None) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    if scores is None:
        scores = arrays.get("signal_scores")
    if labels is None:
        return None
...
def _apply_threshold(labels: np.ndarray, scores: np.ndarray, threshold: float) -> dict[str, Any]:
    ...
    accuracy = float(np.mean(selected.astype(np.int64) == labels)) if labels.size else None
    return {
...
def _validation_threshold_final_test(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if model_val_arrays is None or final_test_arrays is None:
        return {
            "available": False,
            "metric": LOCAL_COMPRESSION_VALIDATION_THRESHOLD_METRIC,
    ...
    applied = _apply_threshold(final_labels, final_scores, threshold)
    return {
        "available": True,
        "metric": LOCAL_COMPRESSION_VALIDATION_THRESHOLD_METRIC,
...
def _flatten_metrics_for_row(row: dict[str, Any], report: Mapping[str, Any]) -> None:
    for split in LOCAL_COMPRESSION_REPORT_SPLITS:
        ...
        if isinstance(binary, Mapping):
            for key, value in binary.items():
                if key == "fpr_at_signal_efficiency":
                    continue
                if isinstance(value, (int, float, str, bool)) or value is None:
                    row[f"{prefix}{key}"] = value
            for key in LOCAL_COMPRESSION_REPORT_BINARY_METRICS:
...
def _write_markdown(path: Path, report: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# Local Compression Particle Transformer Report",
        "",
        f"- ok: {report.get('ok')}",
        ...
        for row in rows:
            lines.append(
                "| {variant} | {fpr50} | {auc} | {acc} | {val_fpr} | {beats} |".format(
                    ...
                    val_fpr=row.get(LOCAL_COMPRESSION_VALIDATION_THRESHOLD_METRIC),
...
class LocalCompressionPartReportConfig:
    """Configuration for the Step 14 local-compression report builder."""
    ...
    variants: tuple[str, ...] = ()
    baseline_variant: str = LOCAL_COMPRESSION_VARIANT_BASELINE_RECHECK
    include_parameter_counts: bool = True
    confirm_final_test: bool = False

    def __post_init__(self) -> None:
        if self.primary_metric is not None and self.primary_metric not in LOCAL_COMPRESSION_SELECTION_METRICS:
            raise ValueError(f"primary_metric must be one of {LOCAL_COMPRESSION_SELECTION_METRICS}")
        if self.comparison_split is not None and self.comparison_split not in LOCAL_COMPRESSION_REPORT_SPLITS:
            raise ValueError(f"comparison_split must be one of {LOCAL_COMPRESSION_REPORT_SPLITS}")
...
def build_local_compression_part_report(config: LocalCompressionPartReportConfig) -> dict[str, Any]:
    """Build Step 14 summary tables from local-compression child run reports."""
    ...
    primary_metric = config.primary_metric or LOCAL_COMPRESSION_PRIMARY_METRIC
    comparison_split = config.comparison_split or "final_test"
    if config.confirm_final_test and comparison_split != "final_test":
        problems.append("confirm_final_test=True but comparison_split is not final_test")
    ...
    for variant, (report_path, payload) in child_payloads.items():
        ...
        model_val_metrics = _metrics_for_split(payload, "model_val")
        final_test_metrics = _metrics_for_split(payload, "final_test")
        final_oracle_fpr = _lookup_metric(final_test_metrics, "fpr_at_signal_eff_0p50")
        row = {
            "variant": variant,
            "source_type": "exact_hlt_part_baseline"
            if variant == LOCAL_COMPRESSION_VARIANT_BASELINE_RECHECK
            else "local_compression_adapter",
            "report_path": str(report_path),
            ...
            "final_test_oracle_fpr_at_signal_eff_0p50": final_oracle_fpr,
            "final_test_background_rejection_at_signal_eff_0p50": _lookup_metric(
                final_test_metrics,
                "background_rejection_at_signal_eff_0p50",
            ),
            LOCAL_COMPRESSION_VALIDATION_THRESHOLD_METRIC: validation_threshold.get("false_positive_rate"),
            ...
            "bootstrap_uncertainty_note": "not implemented for Step 14",
        }
        if row["model_val_selected_metric"] is None:
            row["model_val_selected_metric"] = _lookup_metric(model_val_metrics, str(payload.get("selection_metric") or primary_metric))
    ...
    output_paths = {
        "report_json": str(output_dir / "local_compression_part_final_report.json"),
        "report_markdown": str(output_dir / "local_compression_part_final_report.md"),
    ...
    protocol = default_local_compression_part_protocol().to_dict()
    report = {
        "experiment_step": LOCAL_COMPRESSION_REPORT_STEP,
        "output_contract": LOCAL_COMPRESSION_REPORT_CONTRACT,
        "ok": len(problems) == 0,
        "problems": problems,
        ...
        "comparison_summary": {
            ...
            "binary_primary_rule": (
                "Local-compression binary reports sort by final_test_fpr_at_signal_eff_0p50 "

"""First PFN teacher-logit reconstruction experiment harness."""
...
from .train_particle_flow import TeacherLogitParticleFlowTrainConfig, train_teacher_logit_particle_flow_reco


EXPERIMENT_STEP = "teacher_logit_reco_step7_particle_flow_first_experiment"
DEFAULT_FIRST_EXPERIMENT_SPLITS = ["stack_val"]


@dataclass(frozen=True)
class PredictionComparisonSpec:
    """Saved prediction block to compare with the PFN reconstructor outputs."""
...
@dataclass
class TeacherLogitParticleFlowFirstExperimentConfig:
    """Configuration for the first PFN -> ParT teacher-logit experiment."""
...
def comparison_delta_summary(primary: Dict[str, Any], comparisons: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    """Compute PFN-minus-comparison metric deltas where both metrics exist."""
...
def build_first_experiment_report(
    *,
    config: TeacherLogitParticleFlowFirstExperimentConfig,
    train_report: Dict[str, Any],
    prediction_report: Dict[str, Any],
) -> Dict[str, Any]:
    pfn_summary = prediction_metric_summary(
        name="particle_flow_reco",
    ...
    return {
        ...
        "prediction_metrics": pfn_summary,
        "comparison_metrics": comparison_summaries,
        "comparison_deltas": comparison_delta_summary(pfn_summary, comparison_summaries),
        "fits_final_stacker": False,
        "requested_final_stacker": bool(config.fit_final_stacker),
        "research_questions": {
            "standalone_signal": "Compare particle_flow_reco stack_val metrics against raw-HLT, GT-reco, and PN-reco rows.",
            "fusion_diversity": (
                "Use the saved PFN prediction blocks with independent fusion after Step 8 runners are added, "
        ...
        "leakage_rules": {
            "training": "PFN reconstructor trains on model_train and selects on model_val only.",
...
def run_teacher_logit_particle_flow_first_experiment(
    config: TeacherLogitParticleFlowFirstExperimentConfig,
) -> Dict[str, Any]:
    """Train PFN -> frozen teacher, collect prediction blocks, and write a report."""
    ...
    train_config = TeacherLogitParticleFlowTrainConfig(
    ...
    train_report = train_teacher_logit_particle_flow_reco(train_config)

    predict_config = TeacherLogitParticleFlowPredictionConfig(
    ...
    prediction_report = collect_teacher_logit_particle_flow_predictions(predict_config)

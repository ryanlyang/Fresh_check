"""Training loop for the teacher-logit ParticleNet-style reconstructor."""
...
EXPERIMENT_STEP = "teacher_logit_reco_step5_particle_net_train"
RECONSTRUCTOR_ARCHITECTURE = "particle_net"


@dataclass
class TeacherLogitParticleNetTrainConfig:
    """Configuration for Step 5 ParticleNet-style reconstructor training."""
    ...
    def model_config(self) -> ParticleNetReconstructorConfig:
        return ParticleNetReconstructorConfig(
...
def teacher_logit_particle_net_checkpoint_payload(
    model,
    optimizer,
    *,
    epoch: int,
    config: TeacherLogitParticleNetTrainConfig,
    model_config: ParticleNetReconstructorConfig,
...
def train_teacher_logit_particle_net_reco(
    config: TeacherLogitParticleNetTrainConfig,
    *,
    model=None,
    teacher=None,
    ...
    """Train the Step 5 teacher-logit ParticleNet-style reconstructor."""
    ...
    reconstructor = model or ParticleNetReconstructor(model_config)
    ...
    run_metadata = {
        ...
        "leakage_rule": (
            "Step 5 trains the ParticleNet-style reconstructor only on model_train and selects only on model_val. "
    ...
    for epoch in range(1, int(config.epochs) + 1):
        ...
        payload = teacher_logit_particle_net_checkpoint_payload(
            reconstructor,
            optimizer,
            epoch=epoch,
            config=config,
    ...
    if best_epoch < 0 or not (output_dir / "best_model_val.pt").exists():
        raise FloatingPointError(
            "Teacher-logit ParticleNet reconstructor did not produce a finite model_val total_loss, "

...
- `scripts/cache_prediction_anchored_bridge_logits.py` for publishing only
  detached logits/labels/event identities from allocation RAM;
...
Numerical training/forward/export code is invoked through explicit executable
paths (`PAB_CONSUMER_EXECUTOR`, `PAB_RECONSTRUCTOR_EXECUTOR`,
`PAB_TEACHER_FORWARD_EXECUTOR`, and `PAB_DEPLOYABLE_EXPORT_EXECUTOR`). This
keeps site/model-factory commands separate from immutable publication. Actual
submission refuses to call `sbatch` unless every required path exists and is
not a symlink. Workers pass the reviewed graph/node/RAM workspace to those
executors, then use the repository publication CLIs to enforce paired-median,
weights-only, cache-provenance, and no-dense-field rules. An explicitly
approved final-test additionally requires `PAB_FINAL_TEST_EXECUTOR` and is
called with `--hlt-only` only after the clean reload gate.
...
After setting the reviewed data/artifact variables and the safe executor
paths, actual submission requires an additional explicit switch:

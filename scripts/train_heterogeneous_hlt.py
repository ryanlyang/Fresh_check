#!/usr/bin/env python3
"""Train one heterogeneous fixed-HLT architecture on model_train/model_val."""
...
from jetclass_fresh.hlt_baseline import HLTBaselineTrainConfig, save_json  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--architecture", choices=HETERO_HLT_ARCHITECTURES, required=True)
    ...
    parser.add_argument("--max-train-jets", type=int, default=150000)
...
def main() -> int:
    args = parse_args()
    arch = normalize_architecture_name(args.architecture)
    ...
    report = train_heterogeneous_hlt_model(
        config,
        architecture=arch,

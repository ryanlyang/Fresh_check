#!/usr/bin/env python3
"""Bind the selected-path fairness ledger and Stage-G control tasks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run:
        ...
        if args.task_specs_output:
            specs = build_fairness_task_specs(factory_config_path=output)
            ...
            if destination.exists():
                if destination.read_text(encoding="utf-8") != encoded:
                    raise FileExistsError(
                        "refusing to overwrite different fairness specs"
                    )
            else:
        ...
        json.dumps(
            {
                ...
                "fairness_run_count": 9,
                "expanded_stage_g_job_count": 24,

#!/usr/bin/env python3
"""Verify PD10 Step 2 split and fixed-HLT cache artifacts."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="PD10 five-way split manifest path (.json or .json.gz)",
    )
    parser.add_argument(
        "--hlt-cache-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(layout.step2_audit_dir),
        help="Directory for PD10 Step 2 audit reports",
    )
...
def expected_size_overrides(args: argparse.Namespace) -> tuple[dict[str, int] | None, dict[str, int] | None]:
    model_defaults = pd10_expected_split_sizes()
    stack_defaults = pd10_stack_placeholder_split_sizes()
    model_values = {
...
def main() -> int:
    ...
    result = write_pd10_step2_audit_reports(
        Path(args.manifest),

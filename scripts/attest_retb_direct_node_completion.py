#!/usr/bin/env python3
"""Publish a source-bound completion for one successful direct RETB worker."""
...
from teacher_logit_reco.relation_expert_token_bridge.direct_completion import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-artifact", required=True, action="append", type=Path
    )
    ...
    publication = publish_direct_node_completion(
        campaign=campaign,
        production_graph=graph,
        campaign_root=args.campaign_root,
        node_id=args.node_id,
        output_paths=args.output_artifact,
    )

#!/usr/bin/env python3
"""Train HLT + one DETR/free-slot reconstructed-view sanity taggers."""
...
from teacher_logit_reco.set_matching.detr_slots.sanity_taggers import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--architectures", nargs="+", choices=DETR_SLOT_ENCODER_ARCHITECTURES, default=list(DETR_SLOT_ENCODER_ARCHITECTURES))
    ...
    parser.add_argument("--use-geometry-attention", action="store_true")
...
def base_config_from_args(args: argparse.Namespace) -> FiveViewTaggerTrainConfig:
    first_architecture = args.architectures[0]
    return FiveViewTaggerTrainConfig(
        output_dir=str(Path(args.output_root) / detr_slot_single_reco_variant_name(first_architecture)),
...
def main() -> int:
    ...
    if len(args.architectures) == 1:
        ...
        report = train_detr_slot_single_reco_tagger(
            config,
            architecture=architecture,
            output_dir=config.output_dir,
        )
        print("detr_slot_single_reco_tagger_training_complete:")
        print(f"  architecture: {architecture}")
        print(f"  variant: {detr_slot_single_reco_variant_name(architecture)}")
    ...
    suite = train_detr_slot_single_reco_suite(
        base_config,
        architectures=tuple(args.architectures),
        output_root=args.output_root,
    )
    print("detr_slot_single_reco_tagger_suite_complete:")
    print(f"  output_root: {args.output_root}")
    for architecture in suite["architectures"]:
        report = suite["reports"][architecture]
        print(
            f"  {architecture}: variant={suite['variants'][architecture]} "
            f"best_epoch={report['best_epoch']} "
            f"best_{report['selection_metric']}={report['best_model_selection_metric_value']:.8g}"
        )
    print(f"  report: {Path(args.output_root) / 'single_reco_sanity_suite_report.json'}")
    return 0

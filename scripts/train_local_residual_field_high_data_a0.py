#!/usr/bin/env python3
"""Train one audited from-scratch A0 replicate in the locked 3M study."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest_path = Path(args.study_manifest).resolve()
    manifest = require_high_data_study_manifest(load_json_object(manifest_path))
    output_dir = Path(args.output_dir).resolve()
    expected = Path(manifest["run_dirs"][str(args.seed)]["A0"]).resolve()
    if output_dir != expected:
        raise ValueError(f"A0 output directory differs from manifest: {output_dir} != {expected}")
    config, audit = build_high_data_a0_config(
        load_json_object(manifest["paths"]["a0_source_metadata"]),
        seed=args.seed,
        output_dir=output_dir,
    )
    output_dir.mkdir(parents=True, exist_ok=False)
    save_json(output_dir / "high_data_recipe_audit.json", audit)
    save_json(output_dir / "high_data_train_config.json", {"config": asdict(config)})
    report = dict(train_local_residual_field_tagger(config))
    identity = {
        "high_data_study_manifest_hash": manifest["artifact_hash"],
        "high_data_study_recipe": "A0",
        "seed": int(args.seed),
    ...
    save_json(output_dir / "high_data_completion.json", completion)

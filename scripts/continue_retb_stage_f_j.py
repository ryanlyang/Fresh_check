#!/usr/bin/env python3
"""Publish one completeness-gated RETB Stage F--J task manifest."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.node_id not in MIDDLE_CONTINUATION_MANIFEST_NODES:
        raise ValueError("node is outside the Stage F--J continuation")
    ...
    digest = validate_middle_continuation(
        payload,
        campaign=campaign,
        production_graph=graph,
        campaign_root=args.campaign_root,
    ...
    result: dict[str, object] = {
        ...
        "middle_continuation_bundle_sha256": digest,
    }
    if not args.dry_run:
        result["publication"] = publish_middle_continuation(
            campaign_root=args.campaign_root,
            payload=payload,
        )

#!/usr/bin/env python3
"""Run the first PFN -> ParT teacher-logit reconstruction experiment."""
...
from teacher_logit_reco.pfn_first_experiment import (  # noqa: E402
...
def parse_comparison(text: str) -> PredictionComparisonSpec:
    parts = text.split(":", 2)
    if len(parts) != 3:
        raise argparse.ArgumentTypeError("Comparisons must be formatted name:prediction_dir:model_name")
    name, prediction_dir, model_name = [part.strip() for part in parts]
    if not name or not prediction_dir or not model_name:
        raise argparse.ArgumentTypeError("Comparison name, prediction_dir, and model_name must be nonempty")
    return PredictionComparisonSpec(name=name, prediction_dir=prediction_dir, model_name=model_name)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        help="Optional comparison formatted name:prediction_dir:model_name. Repeat for raw_hlt/gt_reco/pn_reco.",
    )
...
def main() -> int:
    ...
    config = TeacherLogitParticleFlowFirstExperimentConfig(
        ...
        comparison_specs=list(args.comparison),
    )
    report = run_teacher_logit_particle_flow_first_experiment(config)
    print("teacher_logit_particle_flow_first_experiment_complete:")
    print(f"  output_dir: {report['output_dir']}")
    print(f"  model_name: {report['model_name']}")
    print(f"  checkpoint: {report['train_report']['checkpoint']}")
    for split, row in report["prediction_metrics"]["splits"].items():
        metrics = row.get("metrics", {})
        print(
            f"  {split}: n={row.get('n_jets')} "

#!/usr/bin/env python3
"""Bind the complete canonical-target consumer-interface screen."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run:
        ...
        if args.task_specs_output:
            specs = build_consumer_screen_task_specs(
            ...
            if destination.exists():
                if destination.read_text(encoding="utf-8") != encoded:
                    raise FileExistsError(
                        "refusing to overwrite different consumer specs"
                    )
            else:
        ...
        json.dumps(
            {
                ...
                "run_count": len(config["consumer_ids"]),

...
from teacher_logit_reco.train_particle_net import (
    EXPERIMENT_STEP,
    RECONSTRUCTOR_ARCHITECTURE,
    TeacherLogitParticleNetTrainConfig,
    teacher_logit_particle_net_checkpoint_payload,
...
class TeacherLogitParticleNetTrainConfigTests(unittest.TestCase):
    ...
    def test_model_and_loss_config_factories(self):
        cfg = TeacherLogitParticleNetTrainConfig(
            output_dir="out",
            manifest_path="manifest.json.gz",
            hlt_cache_dir="hlt",
            teacher_checkpoint="teacher.pt",
        ...
        self.assertEqual(cfg.model_config().edgeconv_dims, (16, 32))
        self.assertEqual(cfg.model_config().k, 8)
        self.assertEqual(cfg.loss_config().temperature, 3.0)

    def test_checkpoint_payload_records_particle_net_architecture(self):
        cfg = TeacherLogitParticleNetTrainConfig(
            output_dir="out",
            manifest_path="manifest.json.gz",
            hlt_cache_dir="hlt",
        ...
        self.assertEqual(payload["model_config"]["edgeconv_dims"], [16, 32])
...
class TeacherLogitParticleNetTinyTrainTests(unittest.TestCase):
    def test_tiny_training_writes_required_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = train_teacher_logit_particle_net_reco(
                cfg,
                teacher=teacher,

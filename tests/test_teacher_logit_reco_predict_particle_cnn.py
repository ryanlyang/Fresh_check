...
from teacher_logit_reco.predict_particle_cnn import (
...
if TORCH_AVAILABLE:
    ...
    from teacher_logit_reco.train_particle_cnn import EXPERIMENT_STEP as TRAIN_EXPERIMENT_STEP
...
class TeacherLogitParticleCnnPredictionConfigTests(unittest.TestCase):
    def test_final_test_requires_confirmation(self):
        ...
        cfg = TeacherLogitParticleCnnPredictionConfig(
            output_dir="out",
            hlt_cache_dir="hlt",
            reconstructor_checkpoint="reco.pt",
    ...
    def test_default_model_name(self):
        self.assertEqual(default_model_name_for_teacher_architecture("ParticleTransformer"), "pcnn_reco_to_part_teacher")
        self.assertEqual(default_model_name_for_teacher_architecture("pn"), "pcnn_reco_to_pn_teacher")
        self.assertEqual(default_model_name_for_teacher_architecture("PFN"), "pcnn_reco_to_pfn_teacher")
...
class TeacherLogitParticleCnnPredictionTorchTests(unittest.TestCase):
    ...
    def test_checkpoint_loader_reconstructs_particle_cnn_model(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            loaded, payload = load_particle_cnn_reconstructor_checkpoint(path, device=torch.device("cpu"))
            self.assertEqual(payload["epoch"], 2)
            self.assertEqual(payload["reconstructor_architecture"], "particle_cnn")
    ...
    def test_collect_predictions_writes_fusion_blocks(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            cfg = TeacherLogitParticleCnnPredictionConfig(
                ...
                reconstructor_checkpoint="synthetic_pcnn_reco.pt",
                splits=["stack_train", "stack_val"],
                batch_size=2,
                device="cpu",
                amp=False,
            )
            report = collect_teacher_logit_particle_cnn_predictions(
                cfg,
                reconstructor=model,
                teacher=teacher,
                hlt_views=hlt_views,
            ...
            self.assertEqual(model_name, "pcnn_reco_to_pfn_teacher")
            ...
            self.assertEqual(loaded.metadata["model_kind"], "teacher_logit_particle_cnn_reco")
            self.assertEqual(loaded.metadata["training_step"], TRAIN_EXPERIMENT_STEP)
            self.assertEqual(loaded.metadata["reconstructor_architecture"], "particle_cnn")
            self.assertEqual(
                loaded.metadata["reconstructor_ordering_assumption"],

...
from teacher_logit_reco.pfn_first_experiment import (
    EXPERIMENT_STEP,
...
class TeacherLogitParticleFlowFirstExperimentConfigTests(unittest.TestCase):
    def test_config_defaults_to_stack_val_only_without_final_test(self):
        cfg = TeacherLogitParticleFlowFirstExperimentConfig(
            output_dir="out",
            teacher_checkpoint="teacher.pt",
        )
        self.assertEqual(cfg.teacher_architecture, "part")
        self.assertEqual(cfg.resolved_model_name, "pfn_reco_to_part_teacher")
        self.assertEqual(cfg.splits, DEFAULT_FIRST_EXPERIMENT_SPLITS)
        self.assertEqual(cfg.splits, ["stack_val"])
    ...
    def test_prediction_metric_summary_and_deltas(self):
        with tempfile.TemporaryDirectory() as tmp:
            pfn_dir = Path(tmp) / "pfn_predictions"
            hlt_dir = Path(tmp) / "hlt_predictions"
            for split in ("stack_val", "final_test"):
                write_prediction_metadata(pfn_dir, "pfn_reco_to_part_teacher", split, accuracy=0.72, cross_entropy=0.9)
            ...
            primary = prediction_metric_summary(
                ...
                prediction_dir=pfn_dir,
                model_name="pfn_reco_to_part_teacher",
                splits=["stack_val", "final_test"],
            )
    ...
    def test_missing_comparison_metadata_is_reported(self):
        with tempfile.TemporaryDirectory() as tmp:
            summary = prediction_metric_summary(
                name="missing_raw_hlt",
                prediction_dir=Path(tmp) / "missing",
                model_name="hlt_baseline",
                splits=["stack_val"],
            )
            self.assertTrue(summary["splits"]["stack_val"]["missing"])
            self.assertIn("metadata_path", summary["splits"]["stack_val"])

    def test_build_first_experiment_report_includes_comparison_specs(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pfn_dir = root / "experiment" / "predictions"
            comparison_dir = root / "comparison_predictions"
            write_prediction_metadata(pfn_dir, "pfn_reco_to_part_teacher", "stack_val", accuracy=0.73, cross_entropy=0.8)
            ...
            report = build_first_experiment_report(
                ...
                prediction_report={"model_name": "pfn_reco_to_part_teacher"},
            )
            self.assertEqual(report["experiment_step"], EXPERIMENT_STEP)
            self.assertEqual(report["model_name"], "pfn_reco_to_part_teacher")
            self.assertEqual(report["reconstructor_architecture"], "particle_flow")
            self.assertFalse(report["fits_final_stacker"])
            self.assertEqual(report["comparison_metrics"][0]["name"], "pn_reco")
            delta = report["comparison_deltas"]["stack_val"]["pn_reco"]["metric_deltas"]["accuracy"]

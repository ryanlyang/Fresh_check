...
from teacher_logit_reco.predict_particle_flow import (
...
if TORCH_AVAILABLE:
    ...
    from teacher_logit_reco.train_particle_flow import EXPERIMENT_STEP as TRAIN_EXPERIMENT_STEP
...
class TeacherLogitParticleFlowPredictionConfigTests(unittest.TestCase):
    def test_final_test_requires_confirmation(self):
        ...
        cfg = TeacherLogitParticleFlowPredictionConfig(
            output_dir="out",
            hlt_cache_dir="hlt",
            reconstructor_checkpoint="reco.pt",
    ...
    def test_default_model_name(self):
        self.assertEqual(default_model_name_for_teacher_architecture("ParticleTransformer"), "pfn_reco_to_part_teacher")
        self.assertEqual(default_model_name_for_teacher_architecture("pn"), "pfn_reco_to_pn_teacher")
        self.assertEqual(default_model_name_for_teacher_architecture("PFN"), "pfn_reco_to_pfn_teacher")
...
class TeacherLogitParticleFlowPredictionTorchTests(unittest.TestCase):
    ...
    def test_checkpoint_loader_reconstructs_particle_flow_model(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            loaded, payload = load_particle_flow_reconstructor_checkpoint(path, device=torch.device("cpu"))
            self.assertEqual(payload["epoch"], 2)
            self.assertEqual(payload["reconstructor_architecture"], "particle_flow")
            view = make_hlt_view()
    ...
    def test_collect_predictions_writes_fusion_blocks(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            cfg = TeacherLogitParticleFlowPredictionConfig(
                ...
                reconstructor_checkpoint="synthetic_pfn_reco.pt",
                splits=["stack_train", "stack_val"],
                batch_size=2,
                device="cpu",
                amp=False,
            )
            report = collect_teacher_logit_particle_flow_predictions(
                cfg,
                reconstructor=model,
                teacher=teacher,
                hlt_views=hlt_views,
            ...
            self.assertEqual(report["reconstructor_architecture"], "particle_flow")
            self.assertEqual(model_name, "pfn_reco_to_pfn_teacher")
            ...
            self.assertEqual(loaded.metadata["model_kind"], "teacher_logit_particle_flow_reco")
            self.assertEqual(loaded.metadata["training_step"], TRAIN_EXPERIMENT_STEP)
            self.assertEqual(loaded.metadata["reconstructor_architecture"], "particle_flow")
            self.assertEqual(

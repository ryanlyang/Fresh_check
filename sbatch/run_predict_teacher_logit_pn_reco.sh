#!/usr/bin/env bash
# Step 8 helper: collect prediction blocks for one trained teacher-logit PN reconstructor.

#SBATCH --job-name=tlogit_pn_pred
...
ARCHITECTURE="${1:?Usage: sbatch/run_predict_teacher_logit_pn_reco.sh <part|pn|pfn|pcnn>}"
...
OUTPUT_DIR="${TEACHER_LOGIT_PN_PREDICTION_RUN_ROOT}/${ARCHITECTURE}"
RECONSTRUCTOR_CHECKPOINT="${TEACHER_LOGIT_PN_RECO_ROOT}/${ARCHITECTURE}/best_model_val.pt"
...
if ! fresh_is_dry_run; then
  mkdir -p "${TEACHER_LOGIT_PN_PREDICTION_DIR}"
fi

cmd=(
  "${PYTHON_BIN}" "-u" "scripts/predict_teacher_logit_particle_net_reco.py"
  ...
  --batch-size "${TEACHER_LOGIT_PN_PREDICT_BATCH_SIZE}"
  --num-workers "${TEACHER_LOGIT_PN_PREDICT_NUM_WORKERS}"
  --device "${TEACHER_LOGIT_PN_PREDICT_DEVICE}"
  --confirm-final-test
)
fresh_append_flag_if_enabled cmd --no-amp "${NO_AMP:-0}"
fresh_append_flag_if_enabled cmd --overwrite-predictions "${OVERWRITE}"
fresh_append_optional_arg cmd --max-jets-per-split "${TEACHER_LOGIT_PN_MAX_JETS_PER_SPLIT}"

fresh_write_run_config "${OUTPUT_DIR}" "predict_teacher_logit_pn_${ARCHITECTURE}" "${cmd[@]}"
fresh_run "${cmd[@]}"

#!/usr/bin/env bash
# Collect prediction blocks for one reco-domain adapted tagger.

#SBATCH --job-name=crossarch_adapt_pred
...
RECO_ARCHITECTURE="${1:?Usage: sbatch run_crossarch_predict_reco_domain_tagger.sh <gt|pn|pfn|pcnn> <part|pn|pfn|pcnn>}"
TEACHER_ARCHITECTURE="${2:?Usage: sbatch run_crossarch_predict_reco_domain_tagger.sh <gt|pn|pfn|pcnn> <part|pn|pfn|pcnn>}"
...
TAGGER_CHECKPOINT="${CROSSARCH_RECO_DOMAIN_TAGGER_DIR}/${RECO_ARCHITECTURE}/${TEACHER_ARCHITECTURE}/best_model_val.pt"
RUN_OUTPUT_DIR="${CROSSARCH_RECO_DOMAIN_TAGGER_PREDICTION_RUN_DIR}/${MODEL_NAME}"
...
cmd=(
  ...
  --splits "${split_args[@]}"
  --batch-size "${CROSSARCH_RECO_DOMAIN_TAGGER_PREDICT_BATCH_SIZE}"
  --num-workers "${CROSSARCH_RECO_DOMAIN_TAGGER_PREDICT_NUM_WORKERS}"
  --device "${CROSSARCH_RECO_DOMAIN_TAGGER_PREDICT_DEVICE}"
...
fresh_append_optional_arg cmd --max-jets-per-split "${CROSSARCH_RECO_DOMAIN_TAGGER_PREDICT_MAX_JETS_PER_SPLIT}"

fresh_write_run_config "${RUN_OUTPUT_DIR}" "crossarch_reco_domain_tagger_predict_${MODEL_NAME}" "${cmd[@]}"
fresh_run "${cmd[@]}"

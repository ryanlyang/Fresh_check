#!/usr/bin/env bash
# Step 7 helper: train one teacher-logit Global Transformer reconstructor.

#SBATCH --job-name=tlogit_train
...
ARCHITECTURE="${1:?Usage: sbatch/run_train_teacher_logit_gt_reco.sh <part|pn|pfn|pcnn>}"
...
OUTPUT_DIR="${TEACHER_LOGIT_GT_RECO_ROOT}/${ARCHITECTURE}"
...
cmd=(
  "${PYTHON_BIN}" "-u" "scripts/train_teacher_logit_global_transformer_reco.py"
  ...
  --batch-size "${TEACHER_LOGIT_GT_BATCH_SIZE}"
  --epochs "${TEACHER_LOGIT_GT_EPOCHS}"
  --lr "${TEACHER_LOGIT_GT_LR}"
  --weight-decay "${TEACHER_LOGIT_GT_WEIGHT_DECAY}"
  --num-workers "${NUM_WORKERS}"
  --device "${DEVICE}"
  --grad-clip-norm "${GRAD_CLIP_NORM}"
  --early-stop-patience "${TEACHER_LOGIT_GT_EARLY_STOP_PATIENCE}"
...
fresh_append_optional_arg cmd --max-train-batches "${TEACHER_LOGIT_GT_MAX_TRAIN_BATCHES}"
fresh_append_optional_arg cmd --max-val-batches "${TEACHER_LOGIT_GT_MAX_VAL_BATCHES}"
fresh_append_optional_arg cmd --max-train-jets "${TEACHER_LOGIT_GT_MAX_TRAIN_JETS}"
fresh_append_optional_arg cmd --max-val-jets "${TEACHER_LOGIT_GT_MAX_VAL_JETS}"

fresh_write_run_config "${OUTPUT_DIR}" "train_teacher_logit_gt_${ARCHITECTURE}" "${cmd[@]}"
fresh_run "${cmd[@]}"

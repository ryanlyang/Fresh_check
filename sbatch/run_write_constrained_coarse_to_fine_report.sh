...
if fresh_bool_enabled "${CONFIRM_FINAL_TEST}"; then
  fusion_report_path="${CONSTRAINED_C2F_FUSION_DIR}/fusion_final_claim_report.json"
else
  fusion_report_path="${CONSTRAINED_C2F_FUSION_DIR}/fusion_report.json"
fi
fresh_require_file "${fusion_report_path}"
...
cmd=(
  ...
  --fusion-report "${fusion_report_path}"

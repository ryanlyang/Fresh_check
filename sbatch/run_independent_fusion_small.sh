#!/usr/bin/env bash
# Handoff independent fusion: small split sizes, frozen HLT + reco7 predictions.

#SBATCH --job-name=ifuse_small
...
: "${FUSION_STACK_TRAIN_SIZE:=50000}"
: "${FUSION_STACK_VAL_SIZE:=20000}"
: "${FUSION_FINAL_TEST_SIZE:=100000}"

RUN_OUTPUT_DIR="${FUSION_MODEL_LOADING_SMALL_DIR}"
...
fresh_write_run_config \
  "${RUN_OUTPUT_DIR}" \
  "independent_fusion_small_50k_20k_100k" \

#!/usr/bin/env bash
# Collect frozen-teacher prediction blocks for one aggressive reconstructor.

#SBATCH --job-name=crossarch_aggr_reco_pred
...
RECO_ARCHITECTURE="${1:?Usage: sbatch run_crossarch_aggressive_predict_reconstructor.sh <aggt|agpn|agpfn|agpcnn> <part|pn|pfn|pcnn>}"
TEACHER_ARCHITECTURE="${2:?Usage: sbatch run_crossarch_aggressive_predict_reconstructor.sh <aggt|agpn|agpfn|agpcnn> <part|pn|pfn|pcnn>}"
...
PREDICT_SCRIPT="$(fresh_crossarch_aggressive_reco_predict_script "${RECO_ARCHITECTURE}")"
...
RUN_OUTPUT_DIR="${CROSSARCH_AGGRESSIVE_RECO_PREDICTION_RUN_DIR}/${MODEL_NAME}"
...
cmd=(
  ...
  --splits "${split_args[@]}"
  --batch-size "${CROSSARCH_AGGRESSIVE_RECO_PREDICT_BATCH_SIZE}"
  --num-workers "${CROSSARCH_AGGRESSIVE_RECO_PREDICT_NUM_WORKERS}"
  --device "${CROSSARCH_AGGRESSIVE_RECO_PREDICT_DEVICE}"
...
fresh_append_flag_if_enabled cmd --non-strict-checkpoint "${NON_STRICT_RECONSTRUCTOR_CHECKPOINT}"
fresh_append_optional_arg cmd --max-jets-per-split "${CROSSARCH_AGGRESSIVE_RECO_PREDICT_MAX_JETS_PER_SPLIT}"

fresh_write_run_config "${RUN_OUTPUT_DIR}" "crossarch_aggressive_reco_predict_${MODEL_NAME}" "${cmd[@]}"
fresh_run "${cmd[@]}"

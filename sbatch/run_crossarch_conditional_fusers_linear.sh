#!/usr/bin/env bash
# Run fast conditional-evidence fusers anchored on HLT4.

#SBATCH --job-name=crossarch_cond_linear
...
fresh_split_words confusion_pair_count_args "${CROSSARCH_CONDITIONAL_FUSER_CONFUSION_PAIR_COUNTS}"
...
OUTPUT_DIR="${CROSSARCH_CONDITIONAL_FUSER_DIR}/linear"
...
cmd=(
  ...
  --confusion-pair-counts "${confusion_pair_count_args[@]}"
...
fresh_write_run_config "${OUTPUT_DIR}" "crossarch_conditional_evidence_fusers_linear" "${cmd[@]}"
fresh_run "${cmd[@]}"

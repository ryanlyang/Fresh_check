#!/usr/bin/env bash
#SBATCH --job-name=fresh_reco7_variant
...
VARIANT="${1:?Usage: sbatch/run_train_fresh_reco7_variant.sh VARIANT}"
export VARIANT
OUTPUT_DIR="${RECO7_ROOT}/${VARIANT}"
...
fresh_append_optional_arg cmd --stage-a-batch-size "${STAGE_A_BATCH_SIZE}"
fresh_append_optional_arg cmd --stage2-batch-size "${STAGE2_BATCH_SIZE}"
fresh_append_optional_arg cmd --stage-a-epochs "${STAGE_A_EPOCHS}"
fresh_append_optional_arg cmd --stage2-epochs "${STAGE2_EPOCHS}"
...
fresh_write_run_config "${OUTPUT_DIR}" "train_reco7_variant_${VARIANT}" "${cmd[@]}"
fresh_run "${cmd[@]}"
